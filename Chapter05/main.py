...
from utils import get_cloudsql_db, log_operation, OpTypes
...
class MainHandler(webapp2.RequestHandler):
    ...
    def post(self):
        ...
        if file_name and file_content:
            ...
            log_operation(user, OpTypes.FILE_ADDED, datetime.datetime.now())
        self._create_note(user, file_name, real_path)
        log_operation(user, OpTypes.NOTE_CREATED, datetime.datetime.now())
...
class ShrinkHandler(webapp2.RequestHandler):
    ...
    def get(self):
        ...
        log_operation(user, OpTypes.SHRINK_PERFORMED, datetime.datetime.now())
